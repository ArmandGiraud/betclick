...
# local imports
from features_utils import create_numeric, encode_all, scale
...
class ChrunPrep():
    ...
    def change_dtype(data):
        ...
        data.acquisition_channel_id = data.acquisition_channel_id.map(str)
        return data

    @staticmethod
    def check_serials():
        """check if serials are present on disk for inference"""
    ...
    def create_labels(self, data, churn_def: int = 90):
        ...
        days_since_last_activity = data.groupby("customer_key")["transaction_date"]\
            .max().map(lambda x: pd.to_datetime(last_date) - x).dt.days
        has_churned = days_since_last_activity > churn_def
        return has_churned

    @staticmethod
    def drop_leaky_rows(data, churn_date: str = '27/06/2019'):
        """remove rows within the targeting range
        to avoid having future observations in the features"""
